...
m_random_race<-
    ...
    tibble(replicate = 1:67)%>%
  
    mutate(race_r = sample(c("White","Black or African American","Asian","Hispanic or Latino","Two or more races"), size=67, replace = TRUE, prob=c(0.5, 0.15, 0.25, 0.1, 0.00)))%>%
  
  mutate(race_p1 = sample(c("White","Black or African American","Asian","Hispanic or Latino","Two or more races"), size=67, replace = TRUE, prob=c(0.5, 0.15, 0.25, 0.1, 0.00)))%>%
  
  mutate(race_p2 = sample(c("White","Black or African American","Asian","Hispanic or Latino","Two or more races"), size=67, replace = TRUE, prob=c(0.5, 0.15, 0.25, 0.1, 0.00)))%>%
  
  mutate(race_p3 = sample(c("White","Black or African American","Asian","Hispanic or Latino","Two or more races"), size=67, replace = TRUE, prob=c(0.5, 0.15, 0.25, 0.1, 0.00)))%>%
  
  mutate(race_p4 = sample(c("White","Black or African American","Asian","Hispanic or Latino","Two or more races"), size=67, replace = TRUE, prob=c(0.5, 0.15, 0.25, 0.1, 0.00)))%>%
...
```{r}
plotm<- tibble(replicate=1:33)%>%
  select(n_same_race)%>%
  rep_sample_n(size = 33, replace = TRUE, reps = 1000) %>% 
  group_by(replicate) %>% 
  summarize(mean_same_race = mean(n_same_race))%>%
  ggplot(aes(x=mean_same_race))+
  geom_density()+
  geom_vline(xintercept=as.numeric(m%>%
  filter(race_r=="White")%>%
...
m_random_race%>%
  filter(race_r=="Asian")%>%
  select(n_same_race)%>%
  rep_sample_n(size = 18, replace = TRUE, reps = 1000) %>% 
  group_by(replicate) %>% 
  summarize(mean_same_race = mean(n_same_race))%>%
  ggplot(aes(x=mean_same_race))+
  geom_density()+
...
m_random_race%>%
  ...
  select(n_same_race)%>%
  rep_sample_n(size = 11, replace = TRUE, reps = 1000) %>% 
  group_by(replicate) %>% 
  summarize(mean_same_race = mean(n_same_race))%>%
  ggplot(aes(x=mean_same_race))+
  geom_density()+
...
m_random_race%>%
  ...
  select(n_same_race)%>%
  rep_sample_n(size = 7, replace = TRUE, reps = 1000) %>% 
  group_by(replicate) %>% 
  summarize(mean_same_race = mean(n_same_race))%>%
  ggplot(aes(x=mean_same_race))+
  geom_density()+
